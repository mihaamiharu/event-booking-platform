...
- Two local runs, identical verdicts (see below).

## Evidence (local: run 1 and run 2 — identical)
...
Worker `ebp-spike-b-local` at
`https://ebp-spike-b-local.ekkisyam2310.workers.dev` + disposable D1
`ebp-spike-b-preview` (personal account — worker and DB both deleted after;
preview run on a real D1, not local sqlite).
...
Identical to both local runs. Preview verdict confirms the S5 decision above:
naive batch is out, serialized conditional-UPDATE gate is in.

Notes: the first preview burst run hit one transient non-JSON error page under
25-way concurrency (driver crashed before recording; rerun clean). Driver since
hardened to record raw bodies instead of throwing (`src/run.ts`). Statement-error
mid-batch atomicity (constraint violation) was not exercised — irrelevant to the
S5 decision, noted for S5 review. No prod data touched.
