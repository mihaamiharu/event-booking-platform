...
describe("wsp-004 provision seed r1-v1", () => {
  it("provisions TEST-DATA state with verifiable credentials", async () => {
    ...
    const alexBookings = get<{ n: number }>(db, `SELECT COUNT(*) AS n FROM bookings WHERE workspace_id = ? AND user_id = '${sid('user_alex')}'`, w);
    assert.equal(alexBookings.n, 0);

    const mayaBooking = get<{ reference: string; quantity: number; total_idr: number }>(db, `SELECT reference, quantity, total_idr FROM bookings WHERE workspace_id = ? AND user_id = '${sid('user_maya')}'`, w);
    assert.deepEqual(mayaBooking, { reference: "BKG-SEED-MAYA-001", quantity: 2, total_idr: 300000 });
    const item = get<{ unit_price_idr: number; subtotal_idr: number }>(db, `SELECT unit_price_idr, subtotal_idr FROM booking_items WHERE workspace_id = ? AND booking_id = '${sid('booking_maya_design')}'`, w);
    ...
    const meetup = get<{ capacity: number; confirmed_quantity: number }>(db, `SELECT capacity, confirmed_quantity FROM event_sessions WHERE workspace_id = ? AND id = '${sid('sess_meetup_01')}'`, w);
    ...
    const past = get<{ status: string }>(db, `SELECT status FROM event_sessions WHERE workspace_id = ? AND id = '${sid('sess_past_01')}'`, w);
